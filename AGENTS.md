...
- Provider variable surfacing → each capability adapter exposes `extract_variables(raw) -> VariableSurface` (`adapters/canonical.py`); the single endpoint `GET /api/orchestration/connections/{id}/agent-variables` (`get_agent_variables` in `api/provider_listings.py`) dispatches by provider and returns ONE shape. Variables are provider-truth — WATI `customParams`, Bolna prompt `{token}` placeholders — never inferred via body/prompt-text fallbacks for a provider that declares them. Unify the output contract (`VariableSurface` + the endpoint); keep derivation provider-local. New providers implement `extract_variables`; never add a parallel variable path.
...
- Event-source adapters (inbound) → external CRM/clinical events enter at `POST /api/orchestration/webhooks/event/{vendor}/{token}`; the per-trigger `webhook_token` resolves exactly ONE `WorkflowTrigger` → one workflow + tenant (Model A — never a global secret + default tenant). Each vendor owns an `EventSourceAdapter` in `orchestration/adapters/event_source/` (declarative `EVENT_MAP` + coded `normalize_event`), resolved via `resolve_adapter('event_source', vendor)`; `vendor=webhook` is identity passthrough. Canonical event names live in `event_catalog.py` keyed by lowercase `workflow_type` (`crm`/`clinical`); adapter `EVENT_MAP` values MUST be a subset (boot-guarded). The webhook-token generator is the single shared `app/utils/webhook_token.py`. Never map native payloads in routes/engine.
...
- The prod frontend is `nginx.prod.conf` baked into the root multi-stage `Dockerfile` (NOT `Dockerfile.frontend`, which is dev/Vite only); the docs site (Zudoku, `basePath /docs`) is built in that Dockerfile's `docs` stage and copied to `/usr/share/nginx/html/docs`. Two non-obvious nginx rules are load-bearing behind Azure Container Apps: (1) `absolute_redirect off;` — without it nginx bakes the internal app FQDN + `http` into `Location` and bounces the custom domain onto the raw azure URL; (2) `try_files $uri $uri.html $uri/ /docs/index.html;` — Zudoku emits per-page `*.html` and expects clean URLs, so `$uri.html` must precede the SPA fallback or `/docs/<page>` serves `index.html` blank. `APP_BASE_URL` is a backend-only webhook-base setting and is unrelated to FE/docs routing. nginx `/docs/` also sets `no-cache` on HTML, `immutable` on `/docs/assets/`, and `text/plain` on `.md`. Local parity: a `docs` compose service runs `zudoku dev` and the frontend's vite proxies `/docs` to it WITHOUT `changeOrigin` (vite's anti-DNS-rebinding host check blocks the `docs` hostname); restart the `frontend` container to pick up `vite.config.ts` edits — macOS bind-mounts don't fire vite's config watcher.
