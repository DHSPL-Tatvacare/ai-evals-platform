...
def _resolve_agent_key(meta: dict) -> str:
    # Group by the external rep id (LSQ user id) when present, otherwise by the
    # display name. Both ship on every persisted thread; the eval flow no longer
    # mints a synthetic external-agent UUID.
    raw = meta.get("rep_external_id") or meta.get("rep_label") or "unknown"
