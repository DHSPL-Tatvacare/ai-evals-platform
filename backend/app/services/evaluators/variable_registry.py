...
class VariableRegistry:
    ...
    def _register_voice_rx_variables(self) -> None:
        ...
        r(VariableDefinition(
            ...
            "audio", "file", ["voice-rx"], requires_audio=True, example="[Audio file attached]",
        ))
        r(VariableDefinition(
            "transcript", "Original Transcript",
            "Full transcript as [Speaker]: text lines from the listing.",
            "transcript", "text", ["voice-rx"],
            example="[Doctor]: How are you feeling?\n[Patient]: I've been having headaches.",
        ))
        r(VariableDefinition(
            "llm_transcript", "Judge Transcript",
            "Transcript generated by judge AI in Step 1. Requires a prior evaluation.",
            "transcript", "text", ["voice-rx"], requires_eval_output=True,
        ))
        r(VariableDefinition(
            "script_preference", "Script Preference",
            "User preference for output script (devanagari, romanized, auto).",
            "context", "text", ["voice-rx"],
        ))
        r(VariableDefinition(
            "language_hint", "Language Hint",
            "Language hint for the audio (e.g., Hindi, Hinglish).",
            "context", "text", ["voice-rx"],
        ))
        r(VariableDefinition(
            "preserve_code_switching", "Code Switching",
            "Whether to preserve code-switching (yes/no).",
            "context", "text", ["voice-rx"],
        ))
        r(VariableDefinition(
            "original_script", "Detected Script",
            "Detected script of the original transcript.",
            "context", "text", ["voice-rx"], source_types=["upload"],
        ))
        r(VariableDefinition(
            "segment_count", "Segment Count",
            "Number of time-aligned segments in the transcript.",
            "transcript", "number", ["voice-rx"], source_types=["upload"],
        ))
        r(VariableDefinition(
            "speaker_list", "Speakers",
            "Comma-separated list of speakers in the transcript.",
            "transcript", "text", ["voice-rx"], source_types=["upload"],
        ))
        r(VariableDefinition(
            "time_windows", "Time Windows",
            "Time windows from original transcript for segment-aligned transcription.",
            "transcript", "text", ["voice-rx"], source_types=["upload"],
        ))
        r(VariableDefinition(
            "structured_output", "Structured Output (rx)",
            "AI-generated structured data (rx object) from API response.",
            "structured_data", "json", ["voice-rx"], source_types=["api"],
        ))
        r(VariableDefinition(
            "api_input", "API Input",
            "Input payload sent to the API.",
            "structured_data", "text", ["voice-rx"], source_types=["api"],
        ))
        r(VariableDefinition(
            "api_rx", "API Response (Full)",
            "Complete API response including metadata.",
            "structured_data", "json", ["voice-rx"], source_types=["api"],
        ))
        r(VariableDefinition(
            "llm_structured", "LLM Structured Output",
            "Structured data extracted by Judge AI in Step 1. Requires a prior evaluation.",
            "structured_data", "json", ["voice-rx"], requires_eval_output=True,
        ))
        r(VariableDefinition(
            "rx.*", "API Response Fields (dynamic)",
            "Access any nested field in the API response via dot notation: {{rx.vitals.temperature}}.",
            "structured_data", "dynamic", ["voice-rx"], source_types=["api"],
