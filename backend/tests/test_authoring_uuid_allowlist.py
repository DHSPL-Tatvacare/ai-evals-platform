...
_VALID_DEFINITION = {
    'nodes': [
        {'id': 'src', 'type': 'source.event_trigger', 'config': {'event_name': 'demo'}},
...
class UuidAllowlistEnforcementTests(unittest.IsolatedAsyncioTestCase):
    async def test_unauthorized_uuid_is_rejected(self) -> None:
        ...
        decoded = json.loads(await _apply_patch_handler(ctx, _wrap(ops)))
...
class GraphPreflightTests(unittest.IsolatedAsyncioTestCase):
    async def test_remove_only_source_makes_graph_invalid(self) -> None:
...
class EgressFilterTests(unittest.IsolatedAsyncioTestCase):
    async def test_credential_field_in_config_is_blocked(self) -> None:
        # An LLM proposes a config that smuggles `api_key` — the egress
        # filter on the patch payload trips before it reaches the wire.
        ops = [{
            'op': 'update_node_config',
            'node_id': 'sink',
        ...
        decoded = json.loads(await _apply_patch_handler(ctx, _wrap(ops)))
        self.assertEqual(
            decoded['meta']['reason_code'], 'CREDENTIAL_LEAK_BLOCKED',
        )
