...
- ORM tables (55): tenants, tenant_configs, users, refresh_tokens, invite_links, apps, roles, role_app_access, role_permissions, audit_log, listings, files, prompts, schemas, evaluators, chat_sessions, chat_messages, history, settings, adversarial_test_cases, tags, jobs, eval_runs, thread_evaluations, adversarial_evaluations, api_logs, eval_reviews, eval_review_items, evaluation_analytics, report_configs, report_runs, report_artifacts, source_call_records, source_lead_records, source_sync_runs, external_agents, eval_templates, analytics_run_facts, analytics_eval_facts, analytics_criterion_facts, analytics_jobs, agent_tool_logs, analytics_query_cache, analytics_charts, analytics_dashboards, sherlock_runtime_sessions, sherlock_runtime_turns, sherlock_runtime_events, llm_usage, model_pricing, model_aliases, llm_usage_daily_rollup, models_dev_catalog, models_dev_snapshot, scheduled_jobs. Plus `alembic_version` (Alembic-owned, not in `Base.metadata`).
...
- `llm_usage` rows are written by the `LoggingLLMWrapper` during every generation call; `llm_usage_daily_rollup` is rebuilt by `populate-cost-rollup` jobs. Request handlers never write either table directly, and pricing resolution goes through `pricing_cache`.
...
- **Schema-aware groundwork (Roadmap 01 Phase 1, no tables moved yet).** Alembic env runs with `include_schemas=True` and pins `version_table_schema='public'`. Every `CatalogTable` has an `effective_schema` (`pg_schema or 'public'`); helpers (`manifest.lookup_column`, `comment_emitter`, `manifest_validator`, `catalog_tools` `information_schema` queries, SQL-validator schema-prefix recognition, `vocabulary.ColumnTarget`) accept `schema.table.column` qualifiers today. Manifest YAMLs do not declare `pg_schema:` yet — they will start once revision `0006` moves tables into `platform`/`analytics`. See `docs/plans/2026-04-24-implementation-sequence/roadmap-01-foundation-postgres-two-schemas.md` §9.6.
