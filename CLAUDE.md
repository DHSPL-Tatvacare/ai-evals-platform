...
- **All orchestration node `_Config(BaseModel)` classes use `extra='forbid'`** under the `ORCHESTRATION_BUILDER_V2=true` env flag (Phase 14 / Phase D). Strictness is sourced from `app.services.orchestration._config_strictness.strict_node_config_dict()`; the helper returns a default `ConfigDict()` when the flag is off (rollback path). Adding a field to a published-workflow contract is a breaking change — declare it on the Pydantic model AND mirror it in the matching Zod schema in `src/features/orchestration/contracts/nodeConfig.ts` in the same commit. After Phase 16 the Zod side becomes generated and only Pydantic moves. Run `python -m app.services.orchestration.contract_audit --output /tmp/audit.csv --only-published` against prod-like data before flipping the flag on a new environment; zero rows = safe to flip.
