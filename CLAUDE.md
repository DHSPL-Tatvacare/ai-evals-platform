...
1. `configure_logging`
2. `_validate_startup_config` — env-var checks; raises `RuntimeError` if `JWT_SECRET`, `ORCHESTRATION_CONNECTION_KEY`, or `LLM_CREDENTIAL_KEY` is missing or invalid Fernet, or if job-timing values are inconsistent
