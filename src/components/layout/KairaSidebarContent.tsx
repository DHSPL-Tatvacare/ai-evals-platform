...
export function KairaSidebarContent({
  searchPlaceholder,
}: KairaSidebarContentProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const debouncedSearch = useDebounce(searchQuery, 300);
  const navigate = useNavigate();

  const { sessions, currentSession, deleteSession, updateSessionTitle } =
    useKairaChat();
  ...
  const handleSelectSession = useCallback(
    (sessionId: string) => {
      // Navigate to the session URL — KairaBotTabView will read chatId from URL
      // and sync with the store. This makes the URL the source of truth.
