...
export const apiRoutes: ApiRoute[] = [
  ...
  { router: 'inside_sales', prefix: '/api/inside-sales', keyEndpoints: 'GET /agents, GET /calls, GET /leads/{prospect_id}, GET /leads, POST /collections/{source_family}/refresh, GET /leads/{prospect_id}/detail', description: 'Inside Sales collection routes — every read surface (lists + detail + drilldown) is served from the source_call_records / source_lead_records mirror; POST /collections/{source_family}/refresh is the only path that triggers a live LeadSquared pull, via the sync-external-source job.' },
  ...
  { router: 'llm', prefix: '/api/llm', keyEndpoints: 'GET /auth-status, POST /discover-models, GET /models', description: 'LLM-related API endpoints — model discovery and auth status' },
