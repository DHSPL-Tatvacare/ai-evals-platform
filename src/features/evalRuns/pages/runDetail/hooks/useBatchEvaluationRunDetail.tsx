 ...
 * Shared batch-eval run-detail hook for the `Run` shape (thread + adversarial
 * sub-rows, batch banners, progress-driven refetch). Exports the hook plus
 * internal helper components in one file — fast-refresh degrades to a full
 * reload here, accepted tradeoff. */
...
import { AdversarialComparisonPanel } from '@/features/evalRuns/components/AdversarialComparisonPanel';
...
} from '@/features/evalRuns/utils/adversarialRunParams';
import { getCanonicalAdversarialCase } from '@/features/evalRuns/utils/adversarialCanonical';
import type { AppId, Run, ThreadEvalRow, AdversarialEvalRow } from '@/types';
  ...
  return (
    <div className="bg-[var(--surface-warning)] border border-[var(--border-warning)] rounded-md px-4 py-2.5 flex items-center gap-2">
      ...
      <span className="text-sm text-[var(--color-warning)] font-medium">
        Evaluation cancelled after {formatDuration(durationSeconds)}. Partial
        results may be shown below.
      </span>
    </div>
  );
}

function ErrorWarningBanner({
  errors,
  total,
  completed,
}: {
  ...
  return (
    <div className="bg-[var(--surface-warning)] border border-[var(--border-warning)] rounded-md px-4 py-2.5 flex items-center gap-2">
      ...
      <span className="text-sm text-[var(--color-warning)] font-medium">
        {errors} of {total} thread evaluations failed. Results below are from
        the {completed} thread{completed !== 1 ? 's' : ''} that succeeded.
      </span>
    </div>
  );
}
...
/**
 * Hook that drives the run-detail surface for `runShape: 'batch'` apps —
 * kaira-bot today. Owns: `Run`-shape fetch + thread/adversarial sub-row
 * fetch, progress-driven incremental refetch, four batch-flavoured banners,
 * adversarial retry, and the baseline/history tab gates.
 *
 * App identity flows in via `appId` for resource resolution (delete redirect,
 * logs href, settings lookup for adversarial-retry credentials).
 */
export function useBatchEvaluationRunDetail(
  appId: AppId,
  runId: string,
): RunDetailView {
  ...
  const { submit: submitAdversarialRetry, isSubmitting: retryingFailedCases } =
    useSubmitAndRedirect({
      appId,
      label: 'Adversarial Retry',
      successMessage:
        'Adversarial retry submitted. It will appear in the runs list shortly.',
  ...
  const { run, phase, error, setRun } = useRunDetailState<Run>({
    ...
    onRunFetched: async (r) => {
      const [t, a] = await Promise.all([
        fetchRunThreads(r.run_id).catch(() => ({
          evaluations: [] as ThreadEvalRow[],
        })),
        fetchRunAdversarial(r.run_id).catch(() => ({
          evaluations: [] as AdversarialEvalRow[],
        })),
      ]);
  ...
  const elapsed = useElapsedTime(
    activeJob?.startedAt ?? run?.timestamp ?? null,
    isRunActive,
  );
  ...
  const retryableAdversarialEvalIds = useMemo(
    () =>
      adversarialEvals
        .filter((evaluation) =>
          Boolean(
            getCanonicalAdversarialCase(evaluation.result, evaluation).derived
              .isRetryable,
          ),
        )
  ...
  const handleCancel = useCallback(async () => {
    ...
    try {
      ...
      setRun((prev) =>
        prev ? { ...prev, status: 'CANCELLED' as Run['status'] } : prev,
      );
    } catch (e: unknown) {
      notificationService.error(
        e instanceof Error ? e.message : 'Unknown error',
  ...
  const handleRetryFailedCases = useCallback(async () => {
    ...
    if (
      ...
      !canSubmitAdversarialRun(adversarialRetrySettings, run)
    ) {
      notificationService.error(
    ...
    if (retryableAdversarialEvalIds.length === 0) {
      notificationService.info(
        'There are no failed adversarial cases to retry in this run.',
      );
      return;
    }
  ...
  usePoll({
    fn: async () => {
      ...
      if (currentProgress !== lastProgressRef.current) {
        ...
        try {
          const [t, a] = await Promise.all([
            fetchRunThreads(runId).catch(() => ({
              evaluations: [] as ThreadEvalRow[],
            })),
            fetchRunAdversarial(runId).catch(() => ({
              evaluations: [] as AdversarialEvalRow[],
            })),
          ]);
          setThreadEvals(t.evaluations);
  ...
  const retryAction =
    detailConfig.extras.adversarialAxes && isAdversarialRun && !isRunActive ? (
      <>
  ...
  const banners = (
    <>
      ...
      {run.status.toLowerCase() === 'failed' &&
        run.error_message &&
        !isRunActive && <FailureBanner message={run.error_message} />}
  ...
  const headerActions = (
    <RunHeaderActions
              ...
              setRun((current) =>
                current
                  ? {
                      ...current,
                      visibility,
                      shared_by: visibility === 'shared' ? current.shared_by : null,
                      shared_at: visibility === 'shared' ? current.shared_at : null,
                    }
                  : current,
              )
            }
      ...
      reviewContent={
        isInReview || !runIsReviewable ? null : (
          <StartReviewButton runId={run.run_id} />
        )
      }
  ...
  const reportContent = (
    <div className="flex-1 min-h-0 overflow-y-auto">
      <AppReportTab appId={appId} runId={run.run_id} runName={run.name ?? null} />
    </div>
  );
  ...
  const baselineContent =
    detailConfig.extras.adversarialAxes &&
    adversarialEvals.length > 0 &&
    !isRunActive ? (
      <div className="flex-1 min-h-0 overflow-y-auto">
  ...
  // Batch runs apply a stricter gate than `RunDetailTabs`: tabs only mount
  // when the run is reviewable AND the surface isn't in inline-review mode.
...
function describeAdversarialCaseSources(
  batchMetadata?: Record<string, unknown>,
): string[] {
  const sourceSummary = batchMetadata?.case_source_summary as
  ...
  if (
    typeof sourceSummary.generated_count === 'number' &&
    sourceSummary.generated_count > 0
  ) {
  ...
  if (
    typeof sourceSummary.manual_count === 'number' &&
    sourceSummary.manual_count > 0
  ) {
  ...
  if (
    typeof sourceSummary.pinned_count === 'number' &&
    sourceSummary.pinned_count > 0
  ) {
