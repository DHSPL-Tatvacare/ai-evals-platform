...
- `/api/reports/{run_id}` endpoint — aggregator selected by `app_id`
- LLM provider abstraction (`llm_base.py`) for narrative generation
- User's configured LLM settings (provider/model)
...
All stored in `ThreadEvaluation.result` JSON.
...
```python
{
    ...
    "heatmap": {
        "agents": ["uuid", ...],        # row order (sorted by avg score desc)
        "agentNames": ["name", ...],     # display names, same order
        "dimensions": ["call_opening", ...],  # column order
        "maxScores": [10, 15, ...],      # max possible per dimension
        "cells": [[float, ...], ...]     # agents × dimensions avg scores
    }
}
```
...
- Verdict thresholds: Strong ≥80, Good ≥65, Needs Work ≥50, Poor <50
- Dimension color thresholds come from evaluator schema (`green_threshold`, `yellow_threshold`)
...
- Uses user's configured LLM settings (provider/model per `(tenant_id, user_id, app_id)`)
- Non-blocking — report renders data sections immediately, narrative enriches when ready
...
- New component: reuse/extend existing `RuleComplianceTable` if possible, else `ComplianceGatesPanel`
...
| Compliance gates | Extend `RuleComplianceTable` or new | Maybe |
...
2. Route checks `app_id` on the `EvalRun` record
3. If `inside-sales` → dispatches to `InsideSalesAggregator`
4. Aggregator reads all `ThreadEvaluation` rows, joins `lsq_agents` for stable identity
5. Computes full aggregate payload
6. `InsideSalesNarrator` generates AI commentary (non-blocking)
7. Both cached in `EvaluationAnalytics` (scope=`single_run`)
8. Returns combined payload to frontend
...
| `backend/app/services/reports/report_service.py` | Route to `InsideSalesAggregator` when `app_id=inside-sales` |
