...
import { FilterPills, ExportButton, InfoBox } from '@/components';
...
export default function BrainMap() {
  ...
  const d3Refs = useRef<{
    node: d3.Selection<SVGCircleElement, BrainNode, SVGGElement, unknown> | null;
  ...
  const initBrainMap = useCallback(() => {
    ...
    const simulation = d3.forceSimulation<BrainNode>(nodes)
      .force('link', d3.forceLink<BrainNode, BrainLink>(linkData)
        .id((d) => d.id)
        .distance((d) => {
      ...
      .force('charge', d3.forceManyBody<BrainNode>().strength((d) => {
        if (d.type === 'feature') return -400;
        if (d.type === 'file') return -150;
        return -50;
      }))
      .force('center', d3.forceCenter(width / 2, height / 2))
      .force('collision', d3.forceCollide<BrainNode>().radius((d) => (d.radius || 8) + 5));

    d3Refs.current.simulation = simulation;

    // Links
    const linkGroup = g.append('g').attr('class', 'links');
    const link = linkGroup.selectAll<SVGLineElement, BrainLink>('line')
      .data(linkData)
      .enter()
      .append('line')
    ...
    // Drag handlers
    function dragStarted(event: d3.D3DragEvent<SVGCircleElement, BrainNode, BrainNode>, d: BrainNode) {
      if (!event.active) simulation.alphaTarget(0.3).restart();
      d.fx = d.x;
      d.fy = d.y;
    }
    function dragged(event: d3.D3DragEvent<SVGCircleElement, BrainNode, BrainNode>, d: BrainNode) {
      d.fx = event.x;
      d.fy = event.y;
    }
    function dragEnded(event: d3.D3DragEvent<SVGCircleElement, BrainNode, BrainNode>, d: BrainNode) {
    ...
    const node = nodeGroup.selectAll<SVGCircleElement, BrainNode>('circle')
      .data(nodes)
      .enter()
      .append('circle')
      .attr('r', (d) => d.radius || 8)
      ...
      .attr('cursor', 'pointer')
      .attr('opacity', 1)
      .call(d3.drag<SVGCircleElement, BrainNode>()
    ...
    const label = labelGroup.selectAll<SVGTextElement, BrainNode>('text')
      .data(nodes)
      .enter()
      .append('text')
    ...
    node
      .on('mouseenter', function (_event, d) {
        d3.select(this).transition().duration(150).attr('r', (d.radius || 8) * 1.3);
        label.filter((ld) => ld.id === d.id).transition().duration(150).attr('opacity', 1);
      })
      .on('mouseleave', function (_event, d) {
        d3.select(this).transition().duration(150).attr('r', d.radius || 8);
        if (!d._selected) {
          label.filter((ld) => ld.id === d.id && ld.type !== 'feature')
    ...
    node.on('click', function (event, d) {
      ...
      nodes.forEach((n) => { n._selected = false; });
      d._selected = true;
      ...
      if (d.type === 'feature') {
        const featureFiles = nodes.filter((n) => n.type === 'file' && n.feature === d.feature);
        ...
        featureFiles.forEach((f) => {
          const badgeBg = f.layer === 'frontend' ? '#dbeafe; color:#1d4ed8' : f.layer === 'backend' ? '#d1fae5; color:#065f46' : '#ede9fe; color:#5b21b6';
        ...
        const fileMethods = nodes.filter((n) => n.type === 'method' && n.fullPath === d.fullPath);
        ...
        const parentFile = nodes.find((n) => n.type === 'file' && n.fullPath === d.fullPath);
    ...
    svg.on('click', () => {
      ...
      if (infoPanelRef.current) {
        infoPanelRef.current.innerHTML = '<p style="color:var(--text-muted)">Click on a node to see details. Use feature pills above to highlight subgraphs.</p>';
      }
    });
  ...
  useEffect(() => {
    ...
    if (activeFeature === 'all') {
      node.transition().duration(300).attr('opacity', 1).attr('r', (d) => d.radius || 8);
      link.transition().duration(300).attr('stroke-opacity', 0.5);
      label.transition().duration(300).attr('opacity', (d) => d.type === 'feature' ? 1 : 0);
    } else {
      node.transition().duration(300)
        .attr('opacity', (d) => d.feature === activeFeature ? 1 : 0.1)
        .attr('r', (d) => d.feature === activeFeature ? (d.radius || 8) * 1.15 : (d.radius || 8));
  ...
  useEffect(() => {
    ...
    if (activeLayer === 'all') {
      ...
      node.transition().duration(300)
        ...
        .attr('r', (d) => d.layer === activeLayer && d.type !== 'feature' ? (d.radius || 8) * 1.15 : (d.radius || 8));
      link.transition().duration(300)
        .attr('stroke-opacity', (d) => {
          const srcLayer = (d.source as BrainNode).layer;
          const tgtLayer = (d.target as BrainNode).layer;
          return srcLayer === activeLayer || tgtLayer === activeLayer ? 0.6 : 0.05;
        });
      label.transition().duration(300)
        .attr('opacity', (d) => {
          if (d.type === 'feature') return 0.6;
  ...
  useEffect(() => {
    ...
    if (!query) {
      node.transition().duration(200).attr('opacity', 1).attr('r', (d) => d.radius || 8);
      link.transition().duration(200).attr('stroke-opacity', 0.5);
      label.transition().duration(200).attr('opacity', (d) => d.type === 'feature' ? 1 : 0);
      return;
    }
    ...
    nodes.forEach((n) => {
      if (matchingIds[n.id] && n.feature) {
        const featureNode = nodes.find((fn) => fn.type === 'feature' && fn.feature === n.feature);
    ...
    node.transition().duration(200)
      ...
      .attr('r', (d) => matchingIds[d.id] ? (d.radius || 8) * 1.3 : (d.radius || 8));
  ...
  return (
    <div ref={contentRef} className="page-content animate-fade-in-up" data-title="Brain Map">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-2xl font-bold" style={{ color: 'var(--text)' }}>Code Brain Map</h2>
        <ExportButton pageTitle="Brain Map" contentRef={contentRef} />
      </div>
      <InfoBox className="mb-4">
        Interactive force-directed graph showing how features, files, and methods connect across the codebase. Click nodes for details, drag to rearrange, and use filters to isolate subgraphs.
      </InfoBox>
      ...
      <div className="flex gap-4 mb-4 flex-wrap items-center">
        ...
        <FilterPills options={layers} active={activeLayer} onChange={setActiveLayer} />
      </div>

      {/* Feature pills */}
      <FilterPills options={features} active={activeFeature} onChange={setActiveFeature} />
      ...
      >
        <p style={{ color: 'var(--text-muted)' }}>
          Click on a node to see details. Use feature pills above to highlight subgraphs.
        </p>
      </div>
    </div>
  );
