...
function read(rel: string): string | null {
  ...
  if (!existsSync(p)) { warn(`source not found: ${rel}`); return null; }
...
function syncTemplateVars(): boolean {
  ...
  const vars: Array<{
    ...
    apps: string; promptTypes: string; flows: string;
  }> = [];
  ...
  for (const block of blocks) {
    ...
    const appsM = block.match(/\[("(?:[^"\\]|\\.)*"(?:\s*,\s*"(?:[^"\\]|\\.)*")*)\]/);
    const apps = appsM
      ? appsM[1].replace(/"/g, '').split(',').map(s => s.trim()).filter(Boolean).join(', ')
      : '';

    // type mapping
    let type: string;
    if (valueType === 'file') type = 'file';
    else if (block.includes('requires_eval_output=True') || valueType === 'number' || valueType === 'dynamic') type = 'computed';
    else type = 'text';

    // flows from source_types kwarg
    const stM = block.match(/source_types=\[([^\]]*)\]/);
    const flows = stM
      ? stM[1].replace(/"/g, '').split(',').map(s => s.trim()).filter(Boolean).join(', ')
      : 'all';
    ...
    else if (category === 'transcript' && key !== 'llm_transcript' && key !== 'chat_transcript') {
      promptTypes = 'transcription, evaluation';
    } else if (['api_input', 'api_rx'].includes(key)) promptTypes = 'transcription, evaluation';

    vars.push({ name: `{{${key}}}`, type, description, apps, promptTypes, flows });
  }

  if (!vars.length) return false;

  write('templateVars.ts', `export interface TemplateVariable {
  name: string;
...
function syncDbModels(): boolean {
  ...
  const models: Array<{ model: string; table: string; keyColumns: string; description: string }> = [];
  ...
  for (const file of pyFiles) {
    ...
    while ((cm = classRe.exec(src)) !== null) {
      const parents = cm[2];
      if (!parents.includes('Base') && !parents.includes('TimestampMixin') && !parents.includes('UserMixin')) continue;

      const className = cm[1];

      // Scope: from this class to the next top-level class or EOF
      const nextClass = src.indexOf('\nclass ', cm.index + 1);
      const section = nextClass > 0 ? src.substring(cm.index, nextClass) : src.substring(cm.index);
      ...
      const cols: string[] = [];
      const colRe = /^ {4}(\w+):\s*Mapped\[([^\]]+)\]\s*=\s*mapped_column\(([^)]*(?:\([^)]*\)[^)]*)*)\)/gm;
      ...
      while ((cc = colRe.exec(section)) !== null) {
        ...
        else if (rawType.includes('float') || rawType.includes('Float')) typeLabel = 'Float';
  ...
  write('dbModels.ts', `export interface DbModel {
  model: string;
  table: string;
  keyColumns: string;
  description: string;
...
function syncApiRoutes(): boolean {
  ...
  const routes: Array<{ router: string; prefix: string; keyEndpoints: string; description: string }> = [];
  ...
  for (const file of pyFiles) {
    ...
    while ((rm = routerRe.exec(src)) !== null) {
      ...
      const decorRe = new RegExp(`@${varName}\\.(get|post|put|delete|patch)\\("([^"]*)"`, 'g');
      ...
      const routerName = varName === 'router'
        ? file.replace('.py', '')
  ...
  write('apiRoutes.ts', `export interface ApiRoute {
  router: string;
  prefix: string;
  keyEndpoints: string;
  description: string;
...
function verifyBrainMap(): void {
  ...
  if (!existsSync(dataFile)) { warn('brainMap.ts not found'); return; }
...
try {
  if (!syncTemplateVars()) warn('templateVars sync failed — using existing fallback');
} catch (e) {
