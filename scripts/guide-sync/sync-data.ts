...
const DATA = resolve(__dirname, "../../src/features/guide/data");
...
function syncDbModels(): boolean {
  ...
  for (const file of pyFiles) {
    ...
    while ((cm = classRe.exec(src)) !== null) {
      ...
      // Inject mixin columns based on parent classes
      const cols: string[] = [];
      if (parents.includes("TenantUserMixin")) {
        cols.push("tenant_id (UUID FK→tenants.id)");
        cols.push("user_id (UUID FK→users.id)");
      }
